    ...
    try {
        ...
        if (z < heatmap_1.heatmapConfig.minZoom || z > heatmap_1.heatmapConfig.maxZoom) {
            return res.status(404).json({
                success: false,
                message: `Zoom level out of range (${heatmap_1.heatmapConfig.minZoom}-${heatmap_1.heatmapConfig.maxZoom})`
            });
        }
        ...
        res.set({
            ...
            'Cache-Control': `public, max-age=${heatmap_1.heatmapConfig.cacheTTL}, s-maxage=${heatmap_1.heatmapConfig.cacheTTL}, stale-while-revalidate=300`,
            'X-Tile-Coords': `${z}/${x}/${y}`,
...
router.get('/grid/:z/:x/:y.json', auth_1.authMiddleware, async (req, res) => {
    try {
        const z = parseInt(req.params.z);
        const x = parseInt(req.params.x);
        const y = parseInt(req.params.y);
        ...
        const venues = await getHeatMapVenues();
        const grid = await heatmapTileService_1.heatmapTileService.getGrid(z, x, y, venues);
        res.set({
...
router.get('/venues', auth_1.authMiddleware, async (req, res) => {
    try {
        const venues = await getHeatMapVenues();
        res.json({
            success: true,
...
router.post('/precompute', auth_1.authMiddleware, async (req, res) => {
    try {
        const { bounds, zoomLevels } = req.body;
        // Use default bounds if not provided
        const computeBounds = bounds || heatmap_1.heatmapConfig.defaultBounds;
        const computeZooms = zoomLevels || [11, 12, 13, 14, 15, 16];
        console.log('🔥 Starting heat map precomputation...', { computeBounds, computeZooms });
        const venues = await getHeatMapVenues();
        await heatmapTileService_1.heatmapTileService.precomputeTiles(venues, computeBounds, computeZooms);
        res.json({
            success: true,
...
router.post('/clear-cache', auth_1.authMiddleware, async (req, res) => {
    try {
        heatmapTileService_1.heatmapTileService.clearCache();
        res.json({
            success: true,
