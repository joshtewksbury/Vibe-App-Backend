...
const validateImageType = (req: AuthenticatedRequest, res: express.Response, next: express.NextFunction) => {
...
router.get('/venues/:venueId/images', authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
...
router.post('/venues/:venueId/images/upload', authMiddleware, upload.single('image'), validateImageType, async (req: AuthenticatedRequest, res) => {
  try {
    const { venueId } = req.params;
    ...
    if (imageType === 'ICON' || imageType === 'BANNER') {
      await prisma.venueImage.updateMany({
        where: {
          venueId,
          imageType: imageType as VenueImageType,
          isActive: true
        },
        data: {
          isActive: false
    ...
    const uploadResult = await imageStorageService.processAndUpload(
      file.buffer,
      venueId,
      imageType as VenueImageType,
      file.originalname
    );
...
router.put('/venues/:venueId/images/:imageId', authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
...
router.delete('/venues/:venueId/images/:imageId', authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
    const { venueId, imageId } = req.params;
    ...
    if (permanent === 'true') {
      ...
      await Promise.all([
        ...
        imageStorageService.deleteImages([existingImage.url, existingImage.thumbnailUrl].filter(Boolean)),
...
router.get('/venues/images/bulk', authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
...
router.get('/venues/:venueId/images/stats', authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
    const { venueId } = req.params;
...
router.post('/admin/images/test-storage', authMiddleware, async (req: AuthenticatedRequest, res) => {
  try {
    // TODO: Add admin role check
    const isConnected = await imageStorageService.testConnection();
